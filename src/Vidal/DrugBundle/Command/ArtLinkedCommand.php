...
class ArtLinkedCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        $this->setName('vidal:art_linked')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $output->writeln('--- vidal:art_linked started');
        ...
        $articles = empty($art_id)
            ...
            : array($emDrug->getRepository("VidalDrugBundle:Art")->findOneById($art_id));
        ...
        if (!empty($articles)) {
            ...
            foreach ($articles as $i => $article) {
                ...
                if ($bodyLinked != $text) {
                    $id = $article->getId();
                    $pdo->prepare("UPDATE art SET bodyLinked = ? WHERE id = ?")->execute(array($text, $id));
                }
            }
        }

        $output->writeln('--- vidal:art_linked finished');
    }
}