...
class ArticleCommand extends ContainerAwareCommand
{
    ...
    {
        $this->setName('vidal:seo:article')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $output->writeln('<info>--- vidal:seo:article started</info>');
        ...
        if (!empty($articles)) {
            ...
            foreach ($articles as $i => $article) {
                ...
                if ($newText != $baseText) {
                    ...
                    $updatedItems++;
                    $id = $article->getId();
                    $pdo->prepare("UPDATE article SET body = ? WHERE id = ?")->execute(array($newText, $id));
                }
            }
        }

        $output->writeln("<comment>updated {$updatedItems} items</comment>");
        $output->writeln('<info>--- vidal:seo:article finished</info>');
    }
}