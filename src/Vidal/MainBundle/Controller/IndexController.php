...
use Vidal\MainBundle\Entity\Module;
...
class IndexController extends Controller
{
    ...
    public function indexAction(Request $request)
    {
        ...
        $letters = $productService->getLetters('p', false);
     ...
     */
    public function moduleAction($moduleId, $textMode = true, $viaAjax = false)
    {
        $em = $this->getDoctrine()->getManager();
        $module = $em->getRepository('VidalMainBundle:Module')->findOneById($moduleId);

        return array('module' => $module, 'textMode' => $textMode, 'viaAjax' => $viaAjax);
    }

    /**
     * @Route("/module-ajax/{moduleId}", name="module_ajax", options={"expose"=true})
     */
    public function moreNewsAction(Request $request, $moduleId)
    {
        ...
        $html = $this->renderView('VidalMainBundle:Index:module_ajax.html.twig', array('module' => $module));

        return new JsonResponse($html);
    }

    /**
     * @Route("/social-top-buttons", name="social_top_buttons", options={"expose"=true})
     */
    public function socialTopButtonsAction(Request $request)
    {
        if($request->isXmlHttpRequest() == false) {
            throw $this->createNotFoundException();
        }

        $html = $this->renderView('VidalMainBundle:Index:social_top_buttons.html.twig');
