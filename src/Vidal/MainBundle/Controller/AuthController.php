...
class AuthController extends Controller
{
    ...
    public function logoutAction() {
        ...
        $url = $this->container->getParameter('kernel.environment') == 'dev'
            ? '/app_dev.php'
	...
	public function registrationAction(Request $request)
	{
		...
		if ($form->isValid()) {
			...
			if (empty($oldUser)) {
				...
				$this->get('email.service')->send(
					$user->getUsername(),
					array('VidalMainBundle:Email:registration.html.twig', array('user' => $user, 'newPw' => $newPw)),
					'Благодарим за регистрацию на нашем портале!'
				);
	...
	{
		return array('title' => 'Политика конфиденциальности', 'description' =>'Политика конфиденциальности');
	}
	...
	{
		return array('title' => 'Пользовательское соглашение', 'description' =>'Пользовательское соглашение');
	}
	...
	public function confirmPasswordResetAction($userId, $hash)
	{
		...
		$this->get('email.service')->send(
			$user->getUsername(),
			array('VidalMainBundle:Email:confirm_password_reset.html.twig', array('user' => $user, 'newPw' => $newPw)),
			'Новый пароль'
		);
